...
- **賛否スペクトラム分析(フェーズ2)** — クラスタを固定分類ではなく「重み付けから都度生成される
  ビュー」として扱うインタラクティブ再クラスタリング。生コメントから1コールで意見抽出+構造化属性
  付与を行い、各意見に stance 確率分布(7分類+unknown、「非反対≠賛成」を保持)・topic/reason タグを
  付与。スライダー操作では候補グラフの再重み付け+UMAP 再配置で点群が連続再編され(LLM は呼ばない)、
  2D レイアウト上で連結制約付き Ward によりクラスタ数を自由に切り直せます。クラスタ選択 → スタンス
  重み上げでそのクラスタだけが賛否に分裂(focus+context)。フッターの「賛否スペクトラム分析」から。
...
設計書は [docs/DESIGN.md](docs/DESIGN.md)。フェーズ2(賛否スペクトラム分析)の
