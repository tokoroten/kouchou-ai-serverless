// 賛否スペクトラム分析の構造化抽出プロンプト(新規。本家由来ではない)。
// stance は「センチメント」ではなく対象への立場。二重否定を単純反転しない。
// 「反対ではない」=非反対(≠賛成)、「賛成できない」=非賛成(≠反対)を厳守する。

const enrichmentBody = `あなたは意見分析の専門家です。与えられた意見文を分析し、構造化された属性をJSONで出力してください。
...
export const enrichmentPrompt = `${enrichmentBody}
出力はJSONのみ。`;

/**
 * 賛否スペクトラム分析の投入口: 生コメントから「意見抽出 + 構造化属性付与」を1コールで行う結合プロンプト。
 * 通常版の抽出プロンプト(本家移植)を意見分割の指針として前段に置き、
 * 各意見への enrichment 付与を後段で指示する。出力形式は extractedOpinionList ではなく opinions。
 */
export function buildExtractEnrichPrompt(extractionPrompt: string): string {
  return `${extractionPrompt}

---
# 追加タスク: 抽出した各意見への構造化属性付与
上記の方針で意見を抽出したうえで、抽出した意見1つずつに対して、以下の定義に従った構造化属性も付けてください。

${enrichmentBody}
# 出力形式(重要)
これまでの extractedOpinionList 形式ではなく、以下の形式で出力してください。抽出した各意見を1要素とし、"argument" に意見文そのものを入れ、同じ要素に上記の構造化フィールド(target/topics/stance/reasons/conditions/holder/quotedSpeech/commitment/confidence)を持たせて "opinions" 配列にまとめます。意見が無ければ opinions は空配列。出力はJSONのみ。`;
}
...
/** 結合抽出スキーマ: 各意見が argument + enrichment フィールドを持つ opinions 配列 */
export const EXTRACT_ENRICH_SCHEMA = {
  name: "ExtractEnrichResponse",
  schema: {
    type: "object",
    properties: {
      opinions: {
        ...
        items: {
          type: "object",
          properties: { argument: { type: "string" }, ...ENRICHMENT_SCHEMA.schema.properties },
          required: ["argument", ...ENRICHMENT_SCHEMA.schema.required],
          additionalProperties: false,
        },
      },
    },
