...
13. **環境構築の未完了をトップで警告する(2026-07-19)**: 設定の「済/未済」判定が
    `isProviderConfigured`(プロバイダ単位・厳密)と `!resolveEndpoint().baseUrl`
    (スロット単位・緩い)の2系統に分裂しており、後者は**プロバイダのキーを削除しても
    プリセットの baseUrl が残るため「設定済み」と誤判定**していた。`slotReadiness()` /
    `pipelineReadiness()`(`src/types/settings.ts`)に一本化し、
    unset(未設定) / unverified(疎通未確認) / ok の3状態を返す。
    さらに一括疎通確認の結果を `Settings.verification` に永続化する。記録には
    `endpointFingerprint()`(baseUrl + model + API キーのハッシュ)を持たせ、
    設定を変えると自動的に「未確認」に戻る。キー本体は指紋に含めない。
    処理ティアや reasoning effort は到達性を変えないため指紋に含めない(変更しても
    未確認に戻さない)。
    警告 UI は `SetupBanner`(`src/components/SetupBanner.tsx`)に共通化し、
    トップ / 新規作成ウィザード / 賛否スペクトラム新規作成で共用する(後者2つは
    ほぼ同一のバナーが重複していた)。色は新設の `.warn-box`(黄)にした —
    **サンプル閲覧と既存レポートの表示は設定ゼロで動く**ため、赤い `.error-box` だと
    「アプリが壊れている」と誤解させるからで、文言も「新規作成には設定が必要」に限定する。
    実行を止めるのは unset のときだけで、unverified は警告に留めて実行はさせる。
    あわせて、ガードが無かった賛否スペクトラムの「データ準備を実行」ボタン
    (サンプル経由で入ると未設定のまま押せた)にも同じ判定を当てた。
